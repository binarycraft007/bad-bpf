...
static const struct argp_option opts[] = {
	...
	{ "input", 'i', "INPUT", 0,
	  "Text to be replaced in file, max 20 chars" },
	{ "replace", 'r', "REPLACE", 0,
	  "Text to replace with in file, must be same size as -t" },
	{ "target-ppid", 't', "PPID", 0,
...
static error_t parse_arg(int key, char *arg, struct argp_state *state)
{
	...
	case 'i':
		if (strlen(arg) >= text_len_max) {
			fprintf(stderr,
				"Text must be less than %d characters\n",
				filename_len_max);
			argp_usage(state);
		}
	...
	case 'f':
		if (strlen(arg) >= filename_len_max) {
			fprintf(stderr,
				"Filename must be less than %d characters\n",
				filename_len_max);
			argp_usage(state);
		}
...
int main(int argc, char **argv)
{
	...
	if (env.filename[0] == '\x00' || env.input[0] == '\x00' ||
	    env.replace[0] == '\x00') {
	...
	strncpy(skel->rodata->filename, env.filename,
		sizeof(skel->rodata->filename));
	skel->rodata->filename_len = strlen(env.filename);
	skel->rodata->target_ppid = env.target_ppid;

	strncpy(skel->rodata->text_find, env.input,
		sizeof(skel->rodata->text_find));
	strncpy(skel->rodata->text_replace, env.replace,
		sizeof(skel->rodata->text_replace));
	...
	int ret = bpf_map_update_elem(bpf_map__fd(skel->maps.map_prog_array),
	...
	ret = bpf_map_update_elem(bpf_map__fd(skel->maps.map_prog_array),
				  &index, &prog_fd, BPF_ANY);
	if (ret == -1) {
		printf("Failed to add program to prog array! %s\n",
		       strerror(errno));
