...
static const struct argp_option opts[] = {
	...
	{ "detatch", 'd', NULL, 0,
	  "Pin programs to filesystem and exit usermode process" },
	{},
};
...
int main(int argc, char **argv)
{
	...
	if (env.detatch) {
		...
		if (access("/sys/fs/bpf", F_OK) != 0) {
			fprintf(stderr,
				"Make sure bpf filesystem mounted by running:\n");
			fprintf(stderr,
	...
	err = bpf_map_update_elem(bpf_map__fd(skel->maps.map_filename), &index,
				  &file, BPF_ANY);
	if (err == -1) {
	...
	err = bpf_map_update_elem(bpf_map__fd(skel->maps.map_text), &index,
				  &text, BPF_ANY);
	if (err == -1) {
	...
	err = bpf_map_update_elem(bpf_map__fd(skel->maps.map_prog_array),
